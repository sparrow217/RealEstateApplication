...
public class MyServlet extends HttpServlet{ 
	...
	/*The test to see if data is passed*/
		...
		out.println("<h3>Hello Servlet<h1>"); 
		
		
		out.println("The data that was passed   ");
		out.println(requestedQuery);
	...
	/* This is the template from the slides	*/
		...
		PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM re where zipcode=?", ResultSet.TYPE_SCROLL_INSENSITIVE,  ResultSet.CONCUR_READ_ONLY);
		preparedStatement.setInt(1, Integer.parseInt(requestedQuery));
